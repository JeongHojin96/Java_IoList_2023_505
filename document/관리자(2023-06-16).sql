...
사용자 ID : C## 이라는 prefix를 기본적으로 사용해야하고 
system과 관련된 몇가지 사항을 기본으로 접근하지 못하도록 하고있다.

사용자 ID, TABLESPACE 접근 권한을 12c 이전 정책으로 되돌리는 설정

이 설정은 사용자를 만드는 경우 항상 해줘야한다.
*/
...
각각 별도의 권한 정책에 의하여 허가를 해줘야 한다.
학습 목적상 세세한 권한 정책을 부여하기가 불편하여

Oracle 의 SYSDBA(SYSTEM) 권한 보다 한단계 낮음 DBA 권한을 부여한다.
