...
-- iolist User 가 접근(CRUD 실행)할수 있는 Table을 보여달라
...
INSERT INTO tbl_buyer ( buid, buname, butel, buaddr)
VALUES('0000000004', '김길동', '010-4444-4444', '함경도 함응');
...
/*
중간 문자열 검색
Full Text Search : 칼럼의 데이터 문자열을 모두 검색하여 일치하는 데이터 찾기
...
숫자의 사칙연산, 문자열 연결하기 등 간단히 SELECT 명령을 통하여
결과를 확인할 수 있다.

그런데 ORACLE 에서는 FROM(절) 키워드가 생략된 SELECT 명령을 활용하지 않는다.
...
-- 현재 저장된 데이터 주엥서 가장 큰 buid 값은 얼마냐
